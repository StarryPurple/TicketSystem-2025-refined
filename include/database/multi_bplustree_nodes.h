...
class MultiBptInternal : public MultiBptNodeBase {
  struct Storage {
    KeyT key;
    ValueT value;
    page_id_t child;
...
template <class KeyT, class ValueT>
class MultiBptLeaf : public MultiBptNodeBase {
  struct Storage {
    KeyT key;
    ValueT value;
  };
...
public:
  ...
  void split(MultiBptLeaf *rht, page_id_t rht_ptr);

  void coalesce(MultiBptLeaf *rht);

  // lft->size < this->size
  void redistribute_left(MultiBptLeaf *lft);

  // rht->size < this->size
  void redistribute_right(MultiBptLeaf *rht);
